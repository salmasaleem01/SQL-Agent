...
from dotenv import load_dotenv, find_dotenv; load_dotenv(find_dotenv())  # Environment variable loading
import os
...
# Parameters:
#   - model: Gemini model selection (gemini-1.5-pro)
...
agent = initialize_agent(
    ...
    agent=AgentType.ZERO_SHOT_REACT_DESCRIPTION,  # ReAct-style tool selection for non-OpenAI models
...
# Interactive CLI Interface for Safe SQL Operations
# This agent has security guardrails - only SELECT operations allowed
def main():
    print("🔒 SECURE SQL Agent - Only allows READ-ONLY operations!")
    print("✅ Security features: Input validation, SQL injection protection, automatic LIMIT")
    print("Type 'quit' or 'exit' to stop.\n")
    
    while True:
        try:
            # Get user input
            user_input = input("🔍 Safe SQL query: ").strip()
            ...
            # Execute the safe agent with user input
            print("\n🔄 Processing your request (with security checks)...")
